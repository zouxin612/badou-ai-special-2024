...
def MobileNet(input_shape=[224,224,3],depth_multiplier=1,dropout=1e-3,classes=1000):
    ...
    # 28,28,256 -> 28,28,256
    x = _depthwise_conv_block(x, 256, depth_multiplier, strides=(2, 2), block_id=5)
    ...
    #14,14,512 -> 7,7,1024
    x = _depthwise_conv_block(x, 512, depth_multiplier, strides=(2,2),block_id=12)
    x = _depthwise_conv_block(x, 512, depth_multiplier, strides=(2,2),block_id=13)

    #7,7,1024 ->1,1,1024
    x = GlobalAveragePooling2D()(x)
    x = Reshape((1,1,1024),name='reshape_1')(x)
    x = Dropout(dropout,name='droupout')(x)
...
def _depthwise_conv_block(inputs,pointwise_conv_filters,depth_multplier=1,strides=(1,1),block_id=1):

    x = DepthwiseConv2D((3,3),padding='same',depth_multplier=depth_multplier,strides=strides,
    ...
    x = Conv2D(pointwise_conv_filters,(1,1),padding='same',use_bias=False,
               strides=(1,1),name='conv_pw_%d_bn' % block_id)(x)
